...
public sealed class HeadlessReplayRunner
{
    ...
    public ReplayResult RunReplayWithRollbacks(string replayFilePath, int rollbackInterval = 10, int rollbackDepth = 5)
    {
        ...
        var terrainData = composition.TerrainDataService;
        int playerCount = replayManager.PlayerCount;
        ...
        while (currentFrame < maxFrame)
        {
            ...
            if (allInputs.TryGetValue(frame, out var frameInputs))
            {
                for (int p = 0; p < frameInputs.Length; p++)
                {
                    ...
                    if (inp.HasMoveCommand || inp.HasBuildingPlacement || inp.HasAttackMoveCommand ||
                        inp.HasDestroyCommand || inp.HasRepairCommand || inp.HasTrainUnitCommand)
                    {
                        _log.Information("Frame {Frame} P{Player}: Move={Move} Build={Build} Destroy={Destroy}",
                            frame, p, inp.HasMoveCommand, inp.HasBuildingPlacement, inp.HasDestroyCommand);
                    }
                }
            }
