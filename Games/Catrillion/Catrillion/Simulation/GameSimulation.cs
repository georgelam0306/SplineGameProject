...
public sealed class GameSimulation : IDisposable
{
        ...
        SessionSeed sessionSeed)
    {
        ...
        // Initialize profiler with system names
        var systemNames = new string[simTableSystems.Count];
        for (int i = 0; i < simTableSystems.Count; i++)
        {
            systemNames[i] = simTableSystems[i].GetType().Name;
        }
        _profiler = new SimulationProfiler(simTableSystems.Count, systemNames);
        ...
        for (int i = 0; i < simTableSystems.Count; i++)
        {
            _systemNamesForHash[i + 1] = systemNames[i];
        }

        // Initialize center flow service with Command Center building dimensions
        // This allows proper perimeter pathing (building center tile is blocked)
        ref readonly var mapConfig = ref gameData.Db.MapConfigData.FindById(0);
        ref readonly var ccData = ref gameData.Db.BuildingTypeData.FindById((int)BuildingTypeId.CommandCenter);
        int mapCenterTileX = mapConfig.WidthTiles / 2;
        int mapCenterTileY = mapConfig.HeightTiles / 2;
        int ccTileX = mapCenterTileX - ccData.Width / 2;
        int ccTileY = mapCenterTileY - ccData.Height / 2;
        _centerFlowService.Initialize(ccTileX, ccTileY, ccData.Width, ccData.Height);
    }
    ...
    {
        _simWorld.BeginFrame(_gameData.Generation, _gameData.Db);
        ...
        bool profilerEnabled = _profiler.Enabled;
        ...
        for (int systemIndex = 0; systemIndex < _simTableSystems.Count; systemIndex++)
        {
            ...
            if (profilerEnabled)
            {
                ...
                _profiler.RecordSystemTick(systemIndex, endTicks - startTicks);
            }
            else
            {
                system.Tick(in context);
        ...
        if (profilerEnabled)
        {
            _profiler.EndFrame(_currentFrame);
        }
    }
    ...
    public void Reset()
    {
        ...
        // Initialize the center flow service for zombie pathfinding with CC building dimensions
