...
# Run DieDrifterDie in various modes.
# Usage:
...
run_desktop() {
  ...
  local pub="${here}/../Outputs/DieDrifterDie/bin/${cfg}/${tfm}/${rid}/publish"
  local exe="${pub}/DieDrifterDie"
  if [[ "${rid}" == win-* ]]; then exe="${pub}/DieDrifterDie.exe"; fi
  ...
  pushd "${pub}" >/dev/null
  if [[ "$(uname -s)" == "Darwin" ]]; then
    xattr -dr com.apple.quarantine ./libraylib*.dylib ./DieDrifterDie 2>/dev/null || true
    codesign --force --sign - --timestamp=none ./libraylib*.dylib 2>/dev/null || true
    codesign --force --sign - --timestamp=none ./DieDrifterDie 2>/dev/null || true
    env -u DYLD_LIBRARY_PATH DYLD_FALLBACK_LIBRARY_PATH=. ${matchmaking_env} ./DieDrifterDie
  elif [[ "$(uname -s)" == "Linux" ]]; then
    env -u LD_LIBRARY_PATH ${matchmaking_env} ./DieDrifterDie
  else
  ...
  for i in $(seq 1 "$count"); do
    ...
    if [[ "$(uname -s)" == "Darwin" ]]; then
      env -u DYLD_LIBRARY_PATH DYLD_FALLBACK_LIBRARY_PATH=. ${matchmaking_env} \
        ./DieDrifterDie --client-index "${client_index}" --client-count "${count}" &
    elif [[ "$(uname -s)" == "Linux" ]]; then
  ...
  for i in $(seq 1 "$count"); do
    ...
    if [[ "$(uname -s)" == "Darwin" ]]; then
      env -u DYLD_LIBRARY_PATH DYLD_FALLBACK_LIBRARY_PATH=. ${net_env} \
        ./DieDrifterDie --client-index "${client_index}" --client-count "${count}" --skip-menu &
    elif [[ "$(uname -s)" == "Linux" ]]; then
...
run_replay() {
  ...
  if [[ "$(uname -s)" == "Darwin" ]]; then
    ...
    codesign --force --sign - --timestamp=none ./libraylib*.dylib 2>/dev/null || true
    codesign --force --sign - --timestamp=none ./DieDrifterDie 2>/dev/null || true
    env -u DYLD_LIBRARY_PATH DYLD_FALLBACK_LIBRARY_PATH=. ${replay_env} ./DieDrifterDie --skip-menu
  elif [[ "$(uname -s)" == "Linux" ]]; then
    env -u LD_LIBRARY_PATH ${replay_env} ./DieDrifterDie --skip-menu
  else
